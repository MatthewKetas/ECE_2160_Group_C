...
static double critter_stddev(double *values, size_t count, double mean)
{
    double variance = 0.0;
    size_t i;
...
int critter_memory_build_summary(const critter_memory_t *memory, critter_window_summary_t *summary)
{
    ...
    count = memory->count;
    temperatures = (double *)malloc(sizeof(double) * count);
    if (temperatures == NULL)
        return -1;

    min_temp = memory->buffer[(memory->head + memory->capacity - count) % memory->capacity].temperature_c;
    max_temp = min_temp;
    mean = 0.0;
    summary->first_timestamp_s = memory->buffer[(memory->head + memory->capacity - count) % memory->capacity].timestamp_s;
    summary->last_timestamp_s = memory->buffer[(memory->head + memory->capacity - 1U) % memory->capacity].timestamp_s;
    ...
    for (i = 0; i < count; ++i)
    {
        index = (memory->head + memory->capacity - count + i) % memory->capacity;
        ...
        {
            summary->sample_count_by_source[memory->buffer[index].source] += 1U;
        }
    ...
    summary->stddev_temperature_c = critter_stddev(temperatures, count, mean);
    summary->latest_temperature_c = memory->buffer[(memory->head + memory->capacity - 1U) % memory->capacity].temperature_c;
    summary->retained_ratio = (count > 0U) ? (double)count / (double)memory->total_received_samples : 0.0;
